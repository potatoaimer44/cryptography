...
app.post('/api/authenticate', async (req, res) => {
    ...
    try {
        ...
        if (!challenge) return res.status(400).send('No active challenge');
        ...
        const token = jwt.sign({ email: result.rows[0].email, voterID: result.rows[0].voter_id, is_admin: result.rows[0].is_admin }, JWT_SECRET, { expiresIn: '1h' });
        await pool.query('UPDATE voters SET challenge = NULL WHERE voter_id = $1', [voterID]);
        res.json({ token, voterID });
    } catch (error) {
...
app.post('/api/cast-vote', authenticateToken, async (req, res) => {
    const { encryptedVote, signature } = req.body; // Removed aesKey, iv as not used in client
